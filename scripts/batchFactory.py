...
def sampleElementMask(
    spec: ArraySpec, batchSize: int, elementCount, device: torch.device, dtype: torch.dtype
) -> torch.Tensor | None:
    ...
    if spec.sparsityRange != (1.0, 1.0):
        pattern = torch.empty((batchSize, 1), device=device).uniform_(
            spec.sparsityRange[0], spec.sparsityRange[1]
        )
...
def sampleGain(
    ...
    if spec.gainRange != (1.0, 1.0):
        gain = torch.empty((batchSize), device=device, dtype=dtype).uniform_(
            spec.gainRange[0], spec.gainRange[1]
        )
    else:
...
def generateBatch(
    ...
    localPositions = sampleElementPositions(
        spec, batchSize, elementCount, aspectRatio, device, dtype
    )
    ...
    if weightsType == "random":
        ...
        weights = sampleUniformWeights(
            spec, batchSize, trueElementCount, device, dtype, elementMask
        )
