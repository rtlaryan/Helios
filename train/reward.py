...
def patternMatchLoss(
    ...
    responseFlat = normalizePower(batchResponse.flatten(1))
    targetImportance = target.importanceMap.flatten()
    targetPower = normalizePower(target.powerMap.flatten())
...
def batchScore(
    ...
    patternLoss = patternMatchLoss(dBResponse, target, lossType, normalize=True)
